...
def _chunk_scan_bwd_dcb_kernel(
    ...
    # Move pointers
    pid_nheads_per_program = pid_g * (nheads // ngroups) + pid_s * nheads_per_program
    x_ptr += pid_b * stride_x_batch + pid_c * chunk_size * stride_x_seqlen + pid_nheads_per_program * stride_x_head 
    dout_ptr += pid_b * stride_dout_batch + pid_c * chunk_size * stride_dout_seqlen + pid_nheads_per_program * stride_dout_head
    dt_ptr += pid_b * stride_dt_batch + pid_c * stride_dt_chunk + pid_nheads_per_program * stride_dt_head
    dA_cumsum_ptr += pid_b * stride_dA_cs_batch + pid_c * stride_dA_cs_chunk + pid_nheads_per_program * stride_dA_cs_head
    if HAS_DDA_CS:
    ...
    # The dcb is a lower triangular matrix where if cols > rows, then the value is 0
    if pid_n * BLOCK_SIZE_N >= (pid_m + 1) * BLOCK_SIZE_M:
        dcb_ptr += pid_b * stride_dcb_batch + pid_c * stride_dcb_chunk + pid_s * stride_dcb_split + pid_g * stride_dcb_group
    ...
    chunk_size_limit = min(chunk_size, seqlen - pid_c * chunk_size)
    # Make sure the chunk size limit is not greater than the number of rows
    ...
    nheads_iter = min(nheads_per_program, nheads - pid_s * nheads_per_program)
    for h in range(nheads_iter):
        ...
        dt_n = tl.load(
            dt_ptrs,
            mask=offs_n < chunk_size,
            other=0.0,
        ).to(tl.float32)
        # [M, N] * [1, N] -> [M, N]
        dcb *= dt_n[None, :]
        # Why not use mask = ... < chunk_size ?
        dA_cs_m = tl.load(
    ...
    if HAS_SEQ_IDX:
        seq_idx_m = tl.load(seq_idx_ptr + offs_m * stride_seq_idx_seqlen, mask=offs_m < chunk_size_limit, other=-1)
        seq_idx_n = tl.load(seq_idx_ptr + offs_n * stride_seq_idx_seqlen, mask=offs_n < chunk_size_limit_n, other=-1)
...
def _chunk_scan_bwd_dcb(x, dt, dA_cumsum, dout, seq_idx=None, CB=None, ngroups=1):
    ...
    dcb = torch.empty(
        batch, nchunks, nsplits, ngroups, chunk_size, chunk_size,
        device=x.device,
        dtype=torch.float32,
    )
